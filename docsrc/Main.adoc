...
expected to take. Notice how its final move is from the `InAChamberLow` state to
...
.upStreamSequence
For the `upStreamSequence`, note how the final move is from `AChamberHigh`
...
This is a generic message for when a menu is selected. It doesn't serve much
use at this point and may be factored out in the future.

[source,elm]
----
<<main-messages>>=
| OnMenuSelect String String
...
----
<<main-update-case>>=
OnMenuSelect _ menuEntry ->
    ( { model
          | wets_popname = menuEntry
      }
      , Nats.Effect.none
      , Cmd.none
    )
...
StartVessel direction ->
    ...
    , Nats.publish "Wets.1" <|
        Wets.startVessel model.wets_popname license <|
...
.Flow Sensor reactions
[cols="1,2,2,1",options="header",width="80%",role=center]
|===
| Population | Flow Sensor | Affected Chamber | Direction

.2+| Wets_1
| Sensor-F01 | ChamberA | Down

| Sensor-F02 | ChamberA | Up

.4+| Wets_2
| Sensor-F03 | ChamberA | Down

.2+|Sensor-F04
| ChamberA | Down
| ChamberB | Up

| Sensor-F05 | ChamberB | Up
|===
...
flowTarget tstate =
    case tstate of
        AChamberHigh -> Just ( ChamberFlow "ChamberA" Low )
        AChamberLow  -> Just ( ChamberFlow "ChamberA" High )
        _ -> Nothing
----
...
handleFlow model cmd =
    ...
    in
    ( { model
        | chamberStates = model.chamberStates
            |> Animator.go ( Animator.millis 2000 )
                ( setChamber "ChamberA" cmd.name )
      }
    , Delay.sequence
    ...
    Svg.svg
        ...
        (List.append
            [ Svg.rect
                ...
                []
            , UI.chamber 0 40
            , animChamber "ChamberA" model 200
            , UI.chamber 400 80
...
animVessel id model direction =
    case Dict.get model.wets_popname wetsPop of
        Nothing ->
            Svg.text ""
        Just strategy ->
...
wetsMenu model =
    OUI.MenuButton.new
        ...
        (OnMenuSelect "popmenu")
        (Button.new
             model.wets_popname |> Button.outlinedButton
        )
        (OUI.Menu.new identity
             |> OUI.Menu.addItems [ "Wets_1", "Wets_2" ]
             |> OUI.Menu.withIcon
                 (\i ->
                     if i == model.wets_popname then
                         Just Icon.check
                     else
                         Nothing
                 )
