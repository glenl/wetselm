...
the `AtLowerEntry` state. During animation, this will whisk the vessel image
...
.upStreamSequence
For the `upStreamSequence`, note how the final move is from `InChamberHigh`
to `AtUpperEntry` to whisk the vessel image offstage left.
